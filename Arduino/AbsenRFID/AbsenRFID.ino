...
//Network SSID
const char* ssid = "IDK";
const char* password = "oopassword";
...
void loop() {
  ...
  if(digitalRead(BTN_PIN)==1)   //ditekan
  {
     ...
     HTTPClient http ;
     //Get Data
     Link = "http://192.168.100.4/absensi/ubahmode.php";
     http.begin(Link);
  ...
  {
     Serial.println("Jaringan kamu gagal");
     return;
  }

  String Link;
  HTTPClient http;
  Link = "http://192.168.100.4/absensi/kirimkartu.php?nokartu=" + IDTAG;
  http.begin(Link);
